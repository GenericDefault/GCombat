...
self.Entity:SetSolid( SOLID_VPHYSICS )        -- CHEESECAKE!    >:3            
 ...
 function ENT:Think()
	if (self.smoking == false) then
		...
		FireTrail:SetKeyValue("rendercolor","255 255 255")
	...
	if (tr.Hit) then
		if ( self.exploded == false ) then
			if ( self.exploded == false && self.ticking == true ) then
				...
				local attack = cbt_dealhcghit( tr.Entity, 1200, 7, tr.HitPos , tr.HitPos)
				if (attack == 0) then
	...
	self.flightvector = self.flightvector + Vector(math.Rand(-1,1), math.Rand(-1,1),-1)
