...
function ENT:Initialize()   
	...
	self.Inputs = Wire_CreateInputs( self.Entity, { "Fire Artillery AP" , "Fire Artillery HE", "Fire Breach Shell" } )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "trimodehowitzer" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:fireBRshell()

	if (self.ammos > 0) then
		local ent = ents.Create( "breachshell" )
		ent:SetPos( self.Entity:GetPos() +  self.Entity:GetUp() * 60)
		ent:SetAngles( self.Entity:GetAngles() )
		ent:Spawn()
		ent:Activate()
		self.armed = false
		self.reloadtime = CurTime() + 30
...
function ENT:TriggerInput(iname, value)
	...
	if (iname == "Fire Breach Shell") then
		if (value == 1 && self.infire == false) then self.infire = true end
		if (value == 0 && self.infire == true) then self.infire = false end
		self.Entity:NextThink( CurTime() )
		self.shelltype = 3
